...
class PFLStrategy(Strategy):  # ABC
    """Abstract base class for server strategy implementations for personalised FL."""
    ...
    def initialize_parameters(
        ...
        Returns
        -------
        parameters_dict : Optional[Dict[str, Parameters]]
            If parameters are returned, then the server will treat these as the
            initial global (personalised) model parameters.
            Dictionary keys can be e.g. client IDs or cluster IDs, depending on strategy.
        """

    @abstractmethod
    def configure_fit(
        self, server_round: int, parameters_dict: Dict[str, Parameters], client_manager: ClientManager
        ...
        parameters_dict : Dict[str, Parameters]
            Dictionary containing the current (global) personalised model parameters.
        client_manager : ClientManager
            The client manager which holds all currently connected clients.
    ...
    def aggregate_fit(
        ...
        Returns
        -------
        parameters_dict : Tuple[Optional[Dict[str, Parameters]], Dict[str, Scalar]]
            If parameters are returned, then the server will treat these as the
            new (global) personalised model parameters (i.e., it will replace the previous
    ...
    def configure_evaluate(
        ...
        server_round : int
            The current round of federated learning.
        parameters_dict : Dict[str, Parameters]
            The current (global) personalised model parameters.
        client_manager : ClientManager
            The client manager which holds all currently connected clients.
